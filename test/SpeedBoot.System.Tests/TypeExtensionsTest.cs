...
public class TypeExtensionsTest
{
    ...
    [DataRow(typeof(IEnumerable<string>), typeof(List<>), false)]
    [DataRow(typeof(List<string>), typeof(IList), true)]
    [DataRow(typeof(Repository<User>), typeof(IRepository<User>), true)]
    [DataRow(typeof(IRepository<User>), typeof(Repository<User>), false)]
    [DataTestMethod]
    public void TestIsImplementType(Type type1, Type type2, bool expected)
    {
        Assert.AreEqual(expected, type1.IsImplementType(type2));
    }
}
...
public class Repository<TEntity> : IRepository<TEntity> where TEntity : class
{
    ...
    public Repository(string name, int id, bool age, DateTime time, DateOnly dateOnly, TimeOnly timeOnly, long @long, ushort @ushort,
        ulong @ulong, uint @uint, UInt16 @uint16, byte @byte)
        : this(name, id, age, time, dateOnly, timeOnly, @long, @ushort, @ulong, @uint)
    {
    }

    public Repository(string name, int id, bool age, DateTime time, DateOnly dateOnly, TimeOnly timeOnly, long @long, ushort @ushort,
        ulong @ulong, uint @uint, UInt16 @uint16, byte @byte, UInt32 @uint32)
        : this(name, id, age, time, dateOnly, timeOnly, @long, @ushort, @ulong, @uint, @uint16, @byte)
    {
    }

    public Repository(string name, int id, bool age, DateTime time, DateOnly dateOnly, TimeOnly timeOnly, long @long, ushort @ushort,
        ulong @ulong, uint @uint, UInt16 @uint16, byte @byte, UInt32 @uint32, float @float)
        : this(name, id, age, time, dateOnly, timeOnly, @long, @ushort, @ulong, @uint, @uint16, @byte, @uint32)
    {
    }

    public Repository(string name, int id, bool age, DateTime time, DateOnly dateOnly, TimeOnly timeOnly, long @long, ushort @ushort,
        ulong @ulong, uint @uint, UInt16 @uint16, byte @byte, UInt32 @uint32, float @float, double @double)
        : this(name, id, age, time, dateOnly, timeOnly, @long, @ushort, @ulong, @uint, @uint16, @byte, @uint32, @float)
    {
    }

    public Repository(string name, int id, bool age, DateTime time, DateOnly dateOnly, TimeOnly timeOnly, long @long, ushort @ushort,
        ulong @ulong, uint @uint, UInt16 @uint16, byte @byte, UInt32 @uint32, float @float, double @double, decimal @decimal)
